...
esp_err_t IRtx_init(IRtx_t *ir_handle, int gpio) {
    ...
    ESP_ERROR_CHECK(rmt_new_tx_channel(&tx_channel_cfg, &ir_handle->channel));

    rmt_carrier_config_t carrier_cfg = {
        .duty_cycle = 0.33,
        .frequency_hz = 38000, // 38KHz
    };
    ESP_ERROR_CHECK(rmt_apply_carrier(ir_handle->channel, &carrier_cfg));



    ir_handle->transmit_cfg = (rmt_transmit_config_t) {
    ...
    ESP_ERROR_CHECK(rmt_new_ir_nec_encoder(&nec_encoder_cfg, &ir_handle->encoder));

    ESP_ERROR_CHECK(rmt_enable(ir_handle->channel));

    return ret; // ok:)
}

esp_err_t IRtx_transmit(IRtx_t *ir, ir_nec_scan_code_t scan_code) {
    return rmt_transmit(ir->channel, ir->encoder, &scan_code, sizeof(scan_code), &ir->transmit_cfg);
}
